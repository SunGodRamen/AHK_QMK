class Logger {
    LogLevels := { "META": 0, "ERROR": 1, "WARN": 2, "INFO": 3, "DEBUG": 4 }
    ...
    SetLogLevel(level) {
        if (level = "") {
            ...
            if (!this.ValidateLogLevel(level)) {
                error_msg := "Invalid log level provided: " . level
                this.InternalError(error_msg)
    ...
    Message(msg_log_level, msg) {
        ...
        msg_log_level_num := this.ConvertLogLevel(msg_log_level)
        if (msg_log_level_num <= current_log_level) {
